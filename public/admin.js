...
function renderChart(canvasId, type) {
  ...
  // Helper to filter out deleted and uncompleted tasks for analytics
  const filteredTasks = tasksCache.filter(t => !(t.deleted && !t.done));
  ...
  switch (type) {
    case "weekly": {
      ...
      for (let i = 3; i >= 0; i--) {
        ...
        const c = filteredTasks.filter(t => {
    ...
    case "weekdays": {
      ...
      filteredTasks.forEach(t => dataArr[new Date(t.date).getDay()]++);
      data = {
        labels,
        datasets: [{
          data: dataArr,
    ...
    case "perPerson": {
      ...
      const counts = peopleCache.map(p =>
        filteredTasks.filter(t => t.assignedTo === p.id).length
      );
      data = {
        labels,
        datasets: [{
    ...
    case "lazyLegends": {
      const labels = peopleCache.map(p => p.name);
      const counts = peopleCache.map(p =>
        filteredTasks.filter(t => t.assignedTo === p.id && !t.done).length
      );
      data = {
        labels,
        datasets: [{
    ...
    case "slacker9000": {
      ...
      const ages = peopleCache.map(p => {
        const openTasks = filteredTasks.filter(t => t.assignedTo === p.id && !t.done && t.assignedDate);
...
function renderTasks() {
  ...
  // Show only tasks not marked deleted
  const visibleTasks = tasksCache.filter(t => !t.deleted);
...
function renderChart(canvasId, type) {
  ...
  // Filter out tasks that are deleted but NOT done (ignore them)
  const filteredTasks = tasksCache.filter(t => !(t.deleted && !t.done));
